...
description:
    Revisiting a BASIC program I wrote in 1985 on my Sony MSX. What did 'good code' look like 40 years ago and what will today's code look like in 2065?
...
<figure>
    <Image src={programAllVersions} alt="Three versions of a BASIC program printed on perforated dot-matrix paper, showing 1980s computer artifacts."/>
    <figcaption class="text-center">
        Printouts of the three versions of the program in perforated dot matrix printing paper (<a href="https://www.youtube.com/watch?v=inhZC75_mdU">YouTube</a>).
    </figcaption>
</figure>

Reading that code made me feel like Indiana Jones in [Raiders of the Lost Ark](https://en.wikipedia.org/wiki/Raiders_of_the_Lost_Ark) in the map room scene... minus the great looks and with significantly more gray hair than Harrison Ford had back then!
...
<figure>
    ...
    <figcaption class="text-center">
        Harrison Ford as Indiana Jones in '[Raiders of the Lost Ark](https://en.wikipedia.org/wiki/Raiders_of_the_Lost_Ark)' in the map room scene ([YouTube](https://youtu.be/gFDM7JGHGYo?si=W5rTFDCoR-cvIt-S)).
    </figcaption>
</figure>
...
<figure>
  <Image src={hitBitSony} alt="A vintage Sony Hit-Bit HB-75P MSX computer, the author's first machine from 1984."/>
    <figcaption class="text-center">
...
I actually found three versions of the program. I took some pictures of the third version and with the help of [Claude](https://claude.ai/) I got the text of the program in no time, which is amazing come to think of it!

## Code Review

Let me walk you through the code, and the four issues that jump out when I read it today with my modern eyes.
...
<figure>
  <Image src={countriesFrance} alt="Screenshot of the BASIC program running in WebMSX, showing the French flag and country information"/>
</figure>
...
This is basically the textbook definition of [spaghetti code](https://en.wikipedia.org/wiki/Spaghetti_code). Code that is convoluted, hard to understand and therefore, hard to maintain. But that was the way personal computers were programmed in those days, [close to the machine](https://www.mcdbooks.com/books/close-to-the-machine-25th-anniversary-edition).

Back then there were **no named functions** with names conveying some meaning about what the function did. Only line numbers existed to identify code locations. Which meant you'd have to manually hunt down the corresponding line, read and understand the actual code to find out what the code was doing. Imagine trying to fix a bug where pressing "G" shows Austria instead of France. You'd have to trace through dozens of line numbers, mentally keeping track of where each GOTO sends you. Now imagine the program getting 10x larger... or maybe don't. Your head may explode!

Today, we just don't jump to a line number, we use **functions** to create "mini-programs" with clear boundaries.
We use **parameters** to define what information a function needs, and we pass **arguments** when we actually run it:
```python
def display_country_information(country_name, capital, currency, language):
    # country_name, capital, currency, language are the parameters of this function
    print(f"Country: {country_name}")
    print(f"Capital: {capital}")
    # ... and so on

...
# 'Austria', 'Vienna', 'EUR', 'German' are the arguments we pass to the display_country_information function
...
Using global variables causes what in technical lingo is called "shared mutable state" and as a program grows in size and new developers are added to the project global variables make understanding the code and adding changes exponentially more difficult.

In the movie 'Deconstructing Harry (1997)', Woody Allen's character takes an elevator to Hell with several stops in between. For programmers, 'Shared Mutable State' would fit right in as on of the stop names:
...
<figure>
    ...
    <figcaption class="text-center">
        Woody Allen as Harry in the elevator to Hell scene ([YouTube](https://youtu.be/9PBwts6k4aw?si=V3ZW1sVisnJR-P20)) from '[Deconstructing Harry (1997)](https://en.wikipedia.org/wiki/Deconstructing_Harry)'.
    </figcaption>
</figure>

Today we are used to define variables inside functions so that they are "visible" within that secific function:
```python
def display_country_information(country_name, capital, currency, language):
...
These past weeks, I've been using Claude Code to write code for me. Me describing what I want in plain English, Claude proposing a plan, iterating back and forth and reviewing what it generates. It already feels like a different kind of programming. Sitting here in 2026, looking at my own BASIC code from 1985, I can't help but ask: which concepts do I take for granted today that will seem as arcane as `GOTO 20600` today?
...
<figure>
    <a href="https://www.youtube.com/watch?v=8pTEmbeENF4">
        <Image class="mb-0" src={bretVictor} alt="Bret Victor presenting 'The Future of Programming' in 2013, using a simulated overhead projector and transparencies"/>
    </a>
    <figcaption class="text-center">
        Bret Victor's talk "The Future of Programming" ([YouTube](https://www.youtube.com/watch?v=8pTEmbeENF4)).
    </figcaption>
</figure>

Some of the things he says in that talk that tie up with this article:
...
<figure>
    ...
    <figcaption class="text-center">
        "Dynamicland intro, Aug 2024" ([YouTube](https://youtu.be/5Q9r-AEzRMA?si=t89sgIqMEk-Ivcg6)).
    </figcaption>
</figure>
...
If could take a time machine today, powered by a fusion reactor fed from [my local `node_modules` directory](https://www.reddit.com/r/ProgrammerHumor/comments/6s0wov/heaviest_objects_in_the_universe/) and go back in time to 1985, meet my younger self painstakingly typing BASIC code on my Hit Bit Sony, grab him and travel to the future... to both our futures that is... to 2050. Would we [had will have found](https://youtu.be/7z6kVb-oP3s?si=AJbQhACsg3L1Br2F) out that code is no longer needed? That the code we are so used to, has turned into the machine code or the assembler no one ever looks into today anymore.
...
<figure>
    ...
    <figcaption class="text-center">
        Final scene ([YouTube](https://www.youtube.com/watch?v=Fr5o3z2t6xI)) from '[Back to the Future](https://en.wikipedia.org/wiki/Back_to_the_Future)'.
    </figcaption>
</figure>
